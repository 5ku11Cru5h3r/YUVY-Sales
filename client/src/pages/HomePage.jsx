...
const HomePage = () => {
  ...
  useEffect(() => {
    if (!checked.length && priceFilter.min == 0 && priceFilter.max == filtermax) {
      getAllProducts();
    }
    ...
    <Layout title={"UKA SALES- BEST OFFERS"}>
      <div className="row">
        <div className="col-md-3 mt-3">
          ...
          <div className="ms-3">
            {categories?.map((c) => (
              <div className='d-flex flex-column'><Checkbox key={c._id} onChange={(e) => { handleFilter(e.target.checked, c._id) }}>{c.name}</Checkbox>
              </div>
            ))}
          </div>
          <br />
          ...
          <div className="ms-5">
            <Slider range defaultValue={[0, filtermax]} max={filtermax * 2} onChange={(value) => { setPriceFilter({ min: value[0], max: value[1] }) }} />
            <br />
            <input className='w-25 ms-4' type="number" placeholder='Min' onChange={e => setPriceFilter({ ...priceFilter, min: e.target.value })} value={priceFilter.min} />
            <input className='w-25 ms-3' type="number" placeholder='Max' onChange={e => setPriceFilter({ ...priceFilter, max: e.target.value })} value={priceFilter.max} />
          </div>
          <br />
          <br />
          <button className='btn btn-danger ms-4' onClick={() => { window.location.reload() }}>Remove Filters</button>
        </div>
