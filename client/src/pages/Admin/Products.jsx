            ...
            {products.map((p) => (
              <div className="card d-flex m-2" style={{ width: '18rem' }}>
                ...
                <ul className="list-group list-group-flush">
                  ...
                  <li className="list-group-item">Shipping: {p.shipping ? "Yes" : "No"} </li>
                </ul>
                <div className="card-body">
                  < Link to={`/dashboard/admin/product/${p.slug}`} key={p._id} className="card-link"><button className="btn btn-outline-warning btn-sm">Update Product</button></Link>
                  <button onClick={(e) => { handleDelete(e, p._id) }} className="btn btn-outline-danger btn-sm ms-3">Delete Product</button>
                </div>
              </div>
            ))}
